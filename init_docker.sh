...
TERMINUS_PUBLIC_URL=${TERMINUS_PUBLIC_URL:-"http://localhost:6363"}
...
if [ ! -f /app/terminusdb/storage/prefix.db ] && [ "$TERMINUS_ENABLE_WELCOME_SCREEN" = false ]; then
    /app/terminusdb/utils/db_util -s "$TERMINUS_SERVER_NAME" -k "$TERMINUS_ADMIN_PASS" --port "$TERMINUS_SERVER_PORT" --public_url "$TERMINUS_PUBLIC_URL" --autologin="$TERMINUS_AUTOLOGIN" --autoattach="$TERMINUS_AUTOATTACH"
elif [ "$TERMINUS_ENABLE_WELCOME_SCREEN" = false ]; then
    /app/terminusdb/utils/db_util -s "$TERMINUS_SERVER_NAME" -k "$TERMINUS_ADMIN_PASS" --port "$SERVER_PORT" --public_url "$TERMINUS_PUBLIC_URL" --autologin="$TERMINUS_AUTOLOGIN" --autoattach="$TERMINUS_AUTOATTACH" --only-config
fi
...
echo "PUBLIC_URL $TERMINUS_PUBLIC_URL"
