...
immediate_class(X,Database) :-
    ...
    xrdf(X, 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type', 'http://www.w3.org/2000/01/rdf-schema#Class').
immediate_class(X,Database) :-
    database_schema(Database,Schema),
    xrdf(X, 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type', 'http://www.w3.org/2002/07/owl#Class').
...
restriction(R,Database) :-
    database_schema(Database,Schema),
    xrdf(R, 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type', 'http://www.w3.org/2002/07/owl#Restriction').
restriction(R,Database) :-
...
no_immediate_class_SC(Database, Reason) :-
    database_schema(Database,Schema),
    xrdf(P,'http://www.w3.org/1999/02/22-rdf-syntax-ns#type','http://www.w3.org/2002/07/owl#ObjectProperty'),
    domain(P,X,Database),
...
restriction_on_property(CR,P,Database) :-
    database_schema(Database,Schema),
	xrdf(CR,'http://www.w3.org/2002/07/owl#onProperty',P),
...
collect(Database,Database_atom,X,[H|T]) :-
    xrdf(X,'http://www.w3.org/1999/02/22-rdf-syntax-ns#first',H),
    !, % assume one result
    xrdf(X,'http://www.w3.org/1999/02/22-rdf-syntax-ns#rest',Y),
    !, % assume one result
...
sub_class_of(Child,Parent,Database) :-
    database_schema(Database,Schema),
    xrdf(Child, 'http://www.w3.org/2000/01/rdf-schema#subClassOf', Parent).
...
union_of(C,U,Database) :-
    database_schema(Database,Schema),
    xrdf(C,'http://www.w3.org/2002/07/owl#unionOf',ListObj),
...
disjoint_union_of(C,U,Database) :-
    database_schema(Database,Schema),
    xrdf(C,'http://www.w3.org/2002/07/owl#disjointUnionOf',ListObj),
    collect(Database,Schema,ListObj,L),
    member(U,L).
...
intersection_of(C,I,Database) :-
    database_schema(Database,Schema),
    xrdf(C,'http://www.w3.org/2002/07/owl#intersectionOf',ListObj),
    collect(Database,Schema,ListObj,L),
    member(I,L).
...
one_of(CC,X,Database) :-
    database_schema(Database,Schema),
    xrdf(CC,'http://www.w3.org/2002/07/owl#oneOf',ListObj),
...
one_of_list(C,OneList,Database) :-
    database_schema(Database,Schema),
    xrdf(C,'http://www.w3.org/2002/07/owl#oneOf',ListObj),
    collect(Database,Schema,ListObj,OneList).

%% :- rdf_meta complement_of(r,r,o).
complement_of(CC,CN,Database) :-
    database_schema(Database,Schema),
    xrdf(CC,'http://www.w3.org/2002/07/owl#complementOf',CN).

%% :- rdf_meta datatype_complementOf(r,r,o).
datatype_complementOf(CC,CN,Database) :-
    database_schema(Database,Schema),
    xrdf(CC,'http://www.w3.org/2002/07/owl#datatypeComplementOf',CN).

%% :- rdf_meta equivalent_class(r,r,o).
equivalent_class(CC,CE,Database) :-
    database_schema(Database,Schema),
    xrdf(CC,'http://www.w3.org/2002/07/owl#equivalentClass',CE).
...
custom_datatype(X,Database) :-
    database_schema(Database,Schema),
    xrdf(X, 'http://www.w3.org/1999/02/22-rdf-syntax-ns#type', 'http://www.w3.org/2000/01/rdf-schema#Datatype').
...
rdf_property(P,Database) :-
    database_schema(Database,Schema),
    xrdf(P,'http://www.w3.org/1999/02/22-rdf-syntax-ns#type','http://www.w3.org/1999/02/22-rdf-syntax-ns#Property').

%:- rdf_meta datatype_property(r,o).
datatype_property(P,Database) :-
    database_schema(Database,Schema),
    xrdf(P,'http://www.w3.org/1999/02/22-rdf-syntax-ns#type','http://www.w3.org/2002/07/owl#DatatypeProperty').
datatype_property(P,_) :- rdfs_datatype_property(P).

%:- rdf_meta annotation_property(r,o).
annotation_property(P,Database) :-
    database_schema(Database,Schema),
    xrdf(P,'http://www.w3.org/1999/02/22-rdf-syntax-ns#type','http://www.w3.org/2002/07/owl#AnnotationProperty').
...
functional_property(P,Database) :-
    database_schema(Database,Schema),
    xrdf(P,'http://www.w3.org/1999/02/22-rdf-syntax-ns#type','http://www.w3.org/2002/07/owl#FunctionalProperty').

%:- rdf_meta inverse_functional_property(r,o).
inverse_functional_property(P,Database) :-
    database_schema(Database,Schema),
    xrdf(P,'http://www.w3.org/1999/02/22-rdf-syntax-ns#type','http://www.w3.org/2002/07/owl#InverseFunctionalProperty').
...
sub_property_of(X,Y,Database) :-
    database_schema(Database,Schema),
    xrdf(X,'http://www.w3.org/2000/01/rdf-schema#sub_propertyOf',Y).
...
range(P,R,Database) :-
    database_schema(Database,Schema),
    xrdf(P,'http://www.w3.org/2000/01/rdf-schema#range',R).
...
any_range(OP,R,Database) :-
    ...
    xrdf(OP,'http://www.w3.org/2002/07/owl#inverseOf',P),
...
domain(P,D,Database) :-
    database_schema(Database,Schema),
    xrdf(P,'http://www.w3.org/2000/01/rdf-schema#domain',D).
...
label(X,Y,Database) :-
    ...
    xrdf(X, 'http://www.w3.org/2000/01/rdf-schema#label',Y).
label(X,Y,Database) :-
    database_schema(Database,Schema),
    xrdf(X, 'http://www.w3.org/2000/01/rdf-schema#label',Y).
...
comment(X,Y,Database) :-
    database_instance(Database,Instance),
    xrdf(X, 'http://www.w3.org/2000/01/rdf-schema#comment', Y).
comment(X,Y,Database) :-
    database_schema(Database,Schema),
    xrdf(X, 'http://www.w3.org/2000/01/rdf-schema#comment', Y).
...
tcs_tag(X,Y,Database) :-
    database_schema(Database,Schema),
    xrdf(X, 'http://terminusdb.com/schema/tcs#tag', Y).
