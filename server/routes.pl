...
test(fetch_something,[
    ...
    create_context(Branch_Descriptor,Branch_Context),
    with_transaction(
        Branch_Context,
        ask(Branch_Context,
            insert(a,b,c)),
    ...
    with_temp_store(
        (
            user_database_name(admin, "test_local", Name),
            create_db_without_schema(Name, 'test local','a test'),
            resolve_absolute_string_descriptor(
                "admin/test_local/_meta", Database_Descriptor),
            create_context(Database_Descriptor, Database_Context),

            % Wrong URI
            atomic_list_concat([Server,'/pack/admin/test'], Remote_URL),
            with_transaction(
                Database_Context,
            ...
            atomic_list_concat([Server,'/fetch/admin/test'], _Fetch_URI)
